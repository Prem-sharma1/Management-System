...
async function main() {
  console.log('=== RESTORING HARSHIT GAJBHIYE AND HIS 144 TASKS (EXCLUDING SCRIPTS & TL) ===');

  const salt = await bcrypt.genSalt(10);
  const hashedPassword = await bcrypt.hash('Harshit@Ai456', salt);

  // 1. Create or update Harshit Gajbhiye as EMPLOYEE (NOT TL)
  ...
  console.log(`User created/updated: ${harshit.name} (ID: ${harshit.id}, Role: ${harshit.role})`);
...
AID-0079-AI-SCRIPT-1	AID-0079	Sharda Orthopedic	Prepare 1st AI Video Script	03/06/2026	AI Video Lead	Harshit	Complete Task	AI Videos	AI Videos 5`;
  ...
  for (const line of lines) {
    ...
    const workingOn = harshit.name; // Harshit Gajbhiye
  ...
  console.log(`Restored ${importedCT} tasks for ${harshit.name}. Skipped ${skippedScriptCount} Script/TL tasks.`);

  // Verify total tasks for Harshit Gajbhiye
  const finalTasks = await prisma.clientTask.findMany({
    where: { workingOn: harshit.name }
  });
  console.log(`Total ClientTasks assigned to ${harshit.name} in DB: ${finalTasks.length}`);
}
