...
export async function GET() {
  try {
    ...
    // First, clear existing plans if we want to re-seed cleanly
    await prisma.plan.deleteMany({});

    // Bulk insert all plans
    ...
    return NextResponse.json({ message: 'Seed successful', count: created.length, plans: created });
  } catch (error) {
    console.error('Seeding error:', error);
    return NextResponse.json({ error: 'Seed failed', details: error.message, stack: error.stack });
  }
}
